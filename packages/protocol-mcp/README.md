...
Successful calls return the Tool value as `structuredContent`. Denied, provider-error, unsupported, and invalid calls set `isError: true` and return a structured status and error envelope. Operations requiring idempotency receive a stable request-derived key automatically; a caller may provide an explicit key through MCP request metadata at `dev.firedrill/idempotency-key`.
