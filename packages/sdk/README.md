...
`binding.environment` contains only the connection values for the target's declared HTTP or MCP binding. `binding.world` exists only for a declared direct binding. A caller-owned test harness should project these values onto an existing client/tool configuration seam; production agent logic should not import Firedrill or add invocation-specific branches. The callback's owning process keeps its normal model/provider credentials; do not copy them into the Firedrill binding. Spawned command targets receive only host variables explicitly named by their `environmentFromHost` mapping.
