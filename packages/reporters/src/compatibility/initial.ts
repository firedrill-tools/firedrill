...
import type { AssertionResult, EvidenceEntry, RunResult } from "@firedrill/contracts";
...
function terminalReport({ result, evidence }: ReportProjectionInput): string {
  const outcome =
...
function evidenceRows(evidence: readonly EvidenceEntry[]): string {
  return evidence
    .map((entry) => {
      ...
      } else subject = String("action" in entry ? entry.action : entry.kind);
...
function htmlReport({ result, evidence }: ReportProjectionInput): string {
