...
function toolSdkRuntimePath(): string {
  const manifestPath = FRAMEWORK_RESOLVER.resolve("@firedrill/tool-sdk/package.json");
    ...
    return {
      ...
      diagnostics: messages.map((message) => {
        ...
        return diagnostic({
          ...
          suggestion:
            "Keep Tool behavior deterministic and local; use defineToolBehavior/context.fail from @firedrill/tool-sdk for runtime helpers.",
        });
      }),
    };
  }
