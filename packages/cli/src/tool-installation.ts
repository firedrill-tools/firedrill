      ...
      if (npmPackage) {
        ...
        if (!path.startsWith("package/"))
          fail(
            ...
            "Use an archive generated by npm pack --ignore-scripts.",
          );
        path = path.slice(8);
      }
...
export async function installToolSource(options: InstallToolSourceOptions): Promise<ToolInstallation> {
  ...
  try {
    ...
    if (source.kind === "git") {
      ...
      archive = await pack(packageRoot, root, staged, options.signal);
      ...
      if (lstatSync(source.path).isDirectory()) {
        ...
        archive = await pack(source.path, root, staged, options.signal);
      } else {
