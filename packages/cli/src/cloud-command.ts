...
export async function executeCloudCommand(arguments_: readonly string[], io: CliIo): Promise<number> {
  ...
  try {
    let entry: string;
    try {
      entry = createRequire(join(io.cwd, "package.json")).resolve("firedrill/cloud");
    } catch {
      entry = createRequire(import.meta.url).resolve("firedrill/cloud");
    }
    ...
    const message =
      "The optional cloud client is unavailable or failed to start. Install a compatible firedrill client package alongside @firedrill/cli. Local commands remain available without it.";
