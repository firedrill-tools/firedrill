...
import { NodePackageNameSchema, PackageIdSchema, SemverSchema } from "@firedrill-run/contracts";
...
export function createToolPackage(input: {
  ...
  try {
    ...
    write("package.json", {
      ...
      devDependencies: {
        "@firedrill-run/cli": installedCliVersion(),
        "@firedrill-run/tool-sdk": installedCliVersion(),
      },
    });
    ...
    return {
      ...
      created,
      nextSteps: [
        "Install reviewed @firedrill-run/cli and @firedrill-run/tool-sdk packages.",
        "npm run validate",
        "npm test",
        "Choose a license before sharing.",
        "Review package lifecycle scripts before npm pack or publish.",
      ],
    };
  } catch (error) {
