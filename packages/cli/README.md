...
Missing packages require separate installation consent. `--install` authorizes acquisition from an npm package selector, local package directory/archive, or Git repository with an optional package subdirectory. Git branches and tags resolve to exact commits; npm selections resolve to exact versions. Local and Git sources are retained as content-addressed `.firedrill-tools/` archives so another checkout does not depend on the author's original directory. Commit those source archives and provenance files with the dependency manifest and package-manager lock; keep generated `.firedrill/` worlds and reports private. Installation uses npm or pnpm with lifecycle scripts disabled and does not execute Tool behavior. Catalog entries pin exact published package versions; a failed registry lookup remains an actionable error and never substitutes another Tool. Review any completed dependency changes before retrying.
