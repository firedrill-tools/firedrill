  ...
  return (
    <section className="fd-definition__section" data-scroll-section="tool-overrides">
      ...
      <p className="fd-tool-overrides-intro">
        Matching rules are checked from bottom to top. Fixed responses and errors skip the Tool’s original
          ...
          <tbody>
            {rules.map((rule) => (
              <tr key={rule.id}>
                ...
                <td>
                  <div>{rule.when?.actorId === undefined ? "Any actor" : `Actor: ${rule.when.actorId}`}</div>
