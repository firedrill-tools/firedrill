...
`runDrill` executes the configured trial set with reproducible seeds. `runDrillTrial` is the lower-level single-trial seam. Local report projection lives in `@firedrill-run/reporters`.
