...
It is an adapter, not another runtime: repository source is compiled by `@firedrill-run/compiler`, drills run through `@firedrill-run/sdk`, and live or retained state is read from the same per-run SQLite worlds that produce local evidence and reports.
