...
it("pins each connection to caller-resolved addresses and refuses a changed resolution", async () => {
  let hits = 0;
  let resolutions = 0;
  ...
  const origin = `http://browser-policy.invalid:${address.port}`;
  const proxy = await startBrowserProxy({
    origins: new Set([origin]),
    timeoutMs: 1000,
    blocked: () => undefined,
    resolveAddress: async () => {
      if (++resolutions > 1) throw Error("denied");
  ...
  try {
    ...
    expect(await fetchStatus()).toBe(502);
    expect(hits).toBe(1);
