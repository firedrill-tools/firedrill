...
2. `workspace.tool.yaml` and `workspace.js` define one stateful Tool operation and its normal HTTP shape.
3. `empty.scenario.yaml` defines the starting state.
4. `local-agent.target.yaml` tells Firedrill how to start this existing agent process and point its HTTP client at the isolated world.
