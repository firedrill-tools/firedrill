...
import { bundleBrowserTestReport, verifyBrowserTestReport } from "@firedrill-run/browser-tests";
...
const installedBrowser = fileURLToPath(import.meta.resolve("@firedrill-run/browser-tests"));
assert.ok(
...
assert.match(JSON.parse(absent.stdout).message, /@firedrill-run\/browser-tests/);
